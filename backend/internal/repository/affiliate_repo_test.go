...
func TestAffiliateWithdrawClaimsOperationBeforeDeducting(t *testing.T) {
	...
	columnMigration, err := os.ReadFile("../../migrations/240_affiliate_ledger_operation_id.sql")
	require.NoError(t, err)
	require.Contains(t, string(columnMigration), "SET LOCAL lock_timeout = '5s'")
	require.NotContains(t, string(columnMigration), "CREATE UNIQUE INDEX")
	indexMigration, err := os.ReadFile("../../migrations/240_affiliate_ledger_operation_id_index_notx.sql")
	require.NoError(t, err)
	require.Contains(t, string(indexMigration), "CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_user_affiliate_ledger_operation_id")
	require.Contains(t, string(indexMigration), "WHERE operation_id IS NOT NULL")
	require.Less(t, "240_affiliate_ledger_operation_id.sql", "240_affiliate_ledger_operation_id_index_notx.sql")
	nonTx, err := validateMigrationExecutionMode("240_affiliate_ledger_operation_id_index_notx.sql", string(indexMigration))
	require.NoError(t, err)
