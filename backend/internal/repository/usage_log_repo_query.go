...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		safetyBufferingEnabled    sql.NullBool
		safetyBufferingFaster     sql.NullString
		createdAt                 time.Time
	)
	...
	if safetyBufferingEnabled.Valid {
		log.SafetyBufferingEnabled = &safetyBufferingEnabled.Bool
	}
	if safetyBufferingFaster.Valid {
		log.SafetyBufferingFasterModel = &safetyBufferingFaster.String
	}
